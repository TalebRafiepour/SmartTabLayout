 ...
 * [.setViewPager] providing it the ViewPager this
 * layout
 * is being used for.
 *
 *
 ...
 */
open class SmartTabLayout @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyle: Int = 0) : HorizontalScrollView(context, attrs, defStyle) {

    protected val tabStrip: SmartTabStrip
    private val titleOffset: Int
    private var fontAssetSrc: String? = null
    private var customTypeFace = Typeface.DEFAULT_BOLD
    private val tabViewBackgroundResId: Int
    private val tabViewTextAllCaps: Boolean
    private var tabViewTextColors: ColorStateList? = null
    ...
    init {
        ...
        var customTabLayoutId = View.NO_ID
        ...
        fontAssetSrc = a.getString(R.styleable.stl_SmartTabLayout_stl_fontAssetSrc)
        a.recycle()

        setFontAssetSrc(fontAssetSrc)
        this.titleOffset = titleOffset
        this.tabViewBackgroundResId = tabBackgroundResId
        this.tabViewTextAllCaps = textAllCaps
        this.tabViewTextColors = textColors ?: ColorStateList.valueOf(TAB_VIEW_TEXT_COLOR)
        ...
        this.internalTabClickListener = if (clickable) InternalTabClickListener() else null
        ...
        this.tabStrip = SmartTabStrip(context, attrs)
        ...
        addView(tabStrip, FrameLayout.LayoutParams.MATCH_PARENT, FrameLayout.LayoutParams.MATCH_PARENT)

    }

    fun setFontAssetSrc(fontAssetSrc: String?) {
        this.fontAssetSrc = fontAssetSrc
        if (fontAssetSrc != null) {
            try {
                customTypeFace = Typeface.createFromAsset(context.assets, fontAssetSrc)
            } catch (e: Exception) {
                e.printStackTrace()
            }
    ...
    private fun populateTabStrip() {
        ...
        for (i in 0 until adapter!!.count) {
            ...
            val tabView = (if (tabProvider == null)
    ...
    private fun scrollToTab(tabIndex: Int, positionOffset: Float) {
        ...
        if (titleOffset == TITLE_OFFSET_AUTO_CENTER) {
            ...
            if (isLayoutRtl) {
                x = if (tabIndex > 0 || positionOffset > 0) titleOffset else 0
            } else {
                x = if (tabIndex > 0 || positionOffset > 0) -titleOffset else 0
            }
    ...
    private class SimpleTabProvider(context: Context, private val tabViewLayoutId: Int, private val tabViewTextViewId: Int) : TabProvider {
    ...
    private inner class InternalTabClickListener : View.OnClickListener {
    ...
    companion object {
        ...
        private val DEFAULT_DISTRIBUTE_EVENLY = false
        private val TITLE_OFFSET_DIPS = 24
        private val TITLE_OFFSET_AUTO_CENTER = -1
        private val TAB_VIEW_PADDING_DIPS = 16
        private val TAB_VIEW_TEXT_ALL_CAPS = true
        private val TAB_VIEW_TEXT_SIZE_SP = 12
        private val TAB_VIEW_TEXT_COLOR = -0x4000000
        private val TAB_VIEW_TEXT_MIN_WIDTH = 0
        private val TAB_CLICKABLE = true
    }
